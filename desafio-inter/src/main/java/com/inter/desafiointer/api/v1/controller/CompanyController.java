...
public class CompanyController {
    ...
    public ResponseEntity<CompanyResponseListDTO> find () {
        return ResponseEntity.ok(new CompanyResponseListDTO(
                CompanyMapper.entitiesToDTOs(companySearchService.findAll())));
    }

    @ApiOperation(value = "Return company by status")
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Request OK"),
            @ApiResponse(code = 500, message = "Server error"),
    })
    @GetMapping(path = "/{status}", produces="application/json")
    public ResponseEntity<CompanyResponseListDTO> findByStatus (
            @ApiParam(value = "Company status", required = true, allowableValues = "ACTIVE,INACTIVE")
            @NonNull @PathVariable(value = "status") String status) {
        return ResponseEntity.ok(new CompanyResponseListDTO(
                CompanyMapper.entitiesToDTOs(companySearchService.findByStatus(status))));
    }
    ...
    public ResponseEntity<CompanyResponseDTO> create (@RequestBody CompanyCreateDTO companyCreateDTO) {
        Company company = companyPersistenceService.save(CompanyMapper.createDtoToEntity(companyCreateDTO));
        return new ResponseEntity<>(CompanyMapper.entityToDTO(company), HttpStatus.CREATED);
    }

    @ApiOperation(value = "Create a list of companies")
    @ApiResponses(value = {
            @ApiResponse(code = 201, message = "Register was created"),
            @ApiResponse(code = 400, message = "Bad request"),
            @ApiResponse(code = 500, message = "Server error"),
    ...
    public ResponseEntity<CompanyResponseListDTO> createList (@RequestBody CompanyCreateListDTO companyCreateListDTO) {
        List<Company> companies = companyPersistenceService.save(CompanyMapper.dtosToEntities(companyCreateListDTO));
        return new ResponseEntity(CompanyMapper.entitiesToDTOs(companies), HttpStatus.CREATED);
    }
